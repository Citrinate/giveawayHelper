// ==UserScript==
// @name Giveaway Helper RU
// @namespace https://github.com/Citrinate/giveawayHelper
// @description Расширение функционала сайтов раздачи ключей Steam
// @author Citrinate
// @version 2.0.4
      ...
      function createButton(group_data, button_callback, show_name, expected_user) {
        if(typeof expected_user !== "undefined" && !expected_user) {
          ...
          var group_name = group_data.group_name,
            ...
            label = in_group ?
              `Выйти из ${show_name ? group_name}`
              : `Вступить в ${show_name ? group_name}`;
        ...
        if(active_groups.indexOf(group_name) == -1) {
          joinSteamGroup(group_name, group_id, function(success) {
            if(success) {
              ...
              giveawayHelperUI.setButtonLabel(button_id, `Выйти из ${show_name ? group_name}`);
            } else {
          ...
          leaveSteamGroup(group_name, group_id, function(success) {
            if(success) {
              ...
              giveawayHelperUI.toggleButtonClass(button_id);
              giveawayHelperUI.setButtonLabel(button_id, `Вступить в ${show_name ? group_name}`);
            } else {
              giveawayHelperUI.showError(steam_community_down_error);
            }
  ...
  var TwitterHandler = (function() {
    function init() {
      ...
      function createButton(action_data, button_callback, show_name, expected_user, end_time) {
        if(!expected_user) {
          ...
          if(action_data.action == "twitter_follow") {
            ...
            button_callback(
              giveawayHelperUI.buildButton(button_id, `Отписаться${show_name ? ` ${twitter_handle}` : ""}`,
                false,
                function() {
            ...
            button_callback(
              giveawayHelperUI.buildButton(button_id, "Удалить ретвит", false, function() {
                giveawayHelperUI.removeButton(button_id);
      ...
      function deleteTwitterFollow(twitter_handle, twitter_id) {
        if(twitter_id === null) {
          giveawayHelperUI.showError(`Ошибка при отписке от следующего Твиттера:
    ...
    function init() {
      ...
      function createButton(twitch_handle, button_callback, show_name, expected_user) {
        if(!expected_user) {
          ...
          // We're not logged in
          giveawayHelperUI.showError(`Вы должны быть авторизированными на
            <a href="https://www.twitch.tv/login" target="_blank">twitch.tv</a>`);
        } else if(expected_user.user_handle != user_handle) {
          // We're logged in as the wrong user
          giveawayHelperUI.showError(`Вы должны быть авторизированными на Твиче и подключены к Gleam.io:
          ...
          var button_id = "twitch_button_" + button_count++,
            label = `Отписаться${(show_name ? ` ${twitch_handle}` : "")}`;
        ...
        GM_xmlhttpRequest({
          ...
          onload: function(response) {
            if(response.status != 204 && response.status != 200) {
              giveawayHelperUI.showError(`Ошибка при отписке от следующего Твича:
